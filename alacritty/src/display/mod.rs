...
impl Display {
    ...
    pub fn draw<T: EventListener>(
        ...
        if let Some(vi_mode_cursor) = vi_mode_cursor {
            ...
            self.draw_line_indicator(config, &size_info, total_lines, Some(vi_point), line, 2);
    ...
    fn draw_line_indicator(
        ...
        let text = format!(" \u{f718} [{}/{}] {}% ", line, total_lines - 1, percent);
        ...
        if mode == 1 {
            bg = colors.line_indicator.background.unwrap_or(config.ui_config.colors.normal.red);
        } else if mode == 2 {
            bg = colors.line_indicator.background.unwrap_or(config.ui_config.colors.normal.green);
        }
